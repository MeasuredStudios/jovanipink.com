...
export const MainNavItem = motion.custom(styled(Link)`
  ${tw`relative font-bold text-indigo-900 border-b border-transparent hover:text-indigo-900 ml-0 sm:ml-8 mt-3 sm:mt-0`};
  width: max-content;
  ...
  &:before {
    ${tw`absolute w-full bg-theme-450 h-px left-0 invisible`};
    content: '';
    bottom: -1px;
    transform: scaleX(0);
    transition: 0.2s;
...
export const ToogleMainNav = styled.button<StyledProps>`
  ...
  span {
    ...
    &:nth-child(2) {
      ${tw`bg-theme-450 inline-block w-8 h-px`};
